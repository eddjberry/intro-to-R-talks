...
+ To install any package you just do:
...
+ `base` is the quickest to learn and automatically produces certain plots when called on certain objects
...
```{r}
df1 <- as_tibble(list(id = rep(1:30, each = 2),
...
+ The functions you're likely to use most are the `map()` functions
    ...
    + Have versions where you specific the class of object you're expecting back
        + Much 'safer' than the apply family
...
## Recommended resources | Finding vignettes
...
## Misc tips
+ Print something out to the console and create an object by putting brackets round  the command
